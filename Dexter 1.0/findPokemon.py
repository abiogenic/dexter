...
movesData = []
for line in pokemon_moves_csv:
	line = line.rstrip()
	line = line.split(',')
	# append this list of items to a variables called "movesData"
	movesData.append(line)	

movesNamesMatrix = []
for line in moves_csv:
	line = line.rstrip()
	line = line.split(',')
	# append this list of items to a variables called "movesNamesMatrix"
	movesNamesMatrix.append(line)

typesMatrix = []
for line in types_csv:
	line = line.rstrip()
	line = line.split(',')
	# append this list of items to a variables called "typesMatrix"
	typesMatrix.append(line)
...
# 		if line[2] == str(number):
# 			return(stat, line[column])
...
# def findStat(statRequested, number, name):
# 	if statRequested.lower() == "all":
# 		hpStat, atkStat, defStat, spatkStat, spdefStat, speedStat = findAllStatsfromNumber(number)
...
# 	else:
# 		statNameFound, stat = findStatfromNumber(number, statRequested)
